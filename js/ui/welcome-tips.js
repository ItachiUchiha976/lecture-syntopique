...
function showUsageGuide() {
  ...
  const card = el('div', { class: 'dialog usage-card', role: 'dialog', 'aria-modal': 'true' }, [
    ...
    el('div', { class: 'usage-scroll' }, [
      ...
      item('🔍', 'Rechercher', 'Insensible à la <strong>casse et aux accents</strong>. <strong>3 portées</strong> : <strong>Ce livre</strong> (le PDF sélectionné), <strong>Les 2 livres</strong> (en lecture double), <strong>Tous les livres</strong> (tous les PDF importés, ouverts ou non). Quand tu touches un résultat, le mot est <strong>surligné sur la page et le reste pendant ta lecture</strong> (même en faisant défiler) — il s’efface quand tu <strong>rouvres la recherche</strong> 🔍 ou changes de page.'),
      ...
      item('⌨️', 'Si le clavier ne s’affiche pas', 'Pour le <strong>saut de page</strong> et la <strong>recherche</strong>, il arrive (rarement) que le clavier ne se lève pas. Dans ce cas, <strong>ferme complètement l’application puis rouvre-la</strong> depuis l’écran d’accueil : le clavier se réactive. Tes annotations et ta progression sont conservées.'),
      item('⏳', 'OCR à l’ouverture (PDF scannés)', 'Pour qu’un <strong>PDF scanné</strong> soit cherchable, son texte doit d’abord être reconnu (OCR précise). À la première ouverture, un écran d’attente affiche la <strong>progression et le temps restant estimé</strong> ; le livre s’ouvre <strong>automatiquement dès la fin</strong>. Les PDF déjà numériques s’ouvrent tout de suite. Si l’OCR <strong>échoue 3 fois</strong> (PDF illisible), le livre s’ouvre <strong>quand même</strong> (en lecture, sans recherche) et porte un repère <strong>⚠️ OCR échouée</strong> dans la bibliothèque — tu peux relancer avec <strong>Réessayer l’OCR</strong>.'),
      ...
      item('💾', 'Sauvegarde : tout est automatique', 'Ton travail (censures, surlignages, progression, notes vocales) est <strong>enregistré automatiquement sur l’appareil</strong>, au fur et à mesure — rien à presser. Deux boutons à ne pas confondre :<ul class="usage-tools"><li><strong>⤓ Sauvegarder</strong> → un fichier <strong>.json</strong> = copie de secours de <strong>TOUT</strong> ton projet, que tu peux <strong>Restaurer ⤒</strong> plus tard (même sur un autre iPad). À faire <strong>de temps en temps</strong> par sécurité (iOS peut effacer le stockage d’une app peu utilisée).</li><li><strong>📤 Exporter</strong> → un <strong>.pdf</strong> = le <strong>résultat final</strong> d’UN livre (caviardé), à lire/partager. Ce n’est <em>pas</em> une copie de secours (non restaurable).</li></ul>'),
